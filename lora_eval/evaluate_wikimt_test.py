#!/usr/bin/env python3
"""
Evaluate LoRA adapter on WikiMT test split.
Compares baseline vs LoRA-adapted model on WikiMT data.
"""
...
# Test data path - using the validation set for WikiMT
WIKIMT_TEST_JSONL = os.path.join(os.path.dirname(__file__), '..', 'data', 'training', 'clamp3_eval_mtf.jsonl')


def load_test_data(jsonl_path, max_samples=None):
    """Load WikiMT test data."""
    data = []
...
def extract_features(model, tokenizer, patchilizer, entries, device):
    ...
    for entry in tqdm(entries, desc="Processing"):
        # Extract text features (WikiMT uses 'analysis' field)
        description = entry.get('analysis', '')
        if not description:
            continue
        ...
        # Extract music features (WikiMT uses 'filepaths' field)
        filepaths = entry.get('filepaths', [])
        if not filepaths:
            continue
        
        # Use first MTF file
        music_path = filepaths[0]
...
def main():
    ...
    # Load test data
    print(f"\nLoading test data from {WIKIMT_TEST_JSONL}")
    test_data = load_test_data(WIKIMT_TEST_JSONL)
    ...
    output_path = os.path.join(os.path.dirname(__file__), 'wikimt_test_results.json')
